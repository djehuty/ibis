...
private:
	...
	static dchar _translateShiftToChar[] = [
		Key.Code.SingleQuote: '\u00b3', // Superscript three
		Key.Code.One: '1',
		Key.Code.Two: '2',
		Key.Code.Three: '3',
		Key.Code.Four: '4',
  ...
  void translate(Key key, out dchar ret, ref dchar dead) {
		ret = '\0';
		...
		if (!(key.shift ^ key.capsLock) && !key.alt && !key.control) {
			...
			if (key.code == Key.Code.Backslash) {
				dead = '\u0300'; // grave
			}
